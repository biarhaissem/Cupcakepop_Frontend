...
export class FormularioComponent implements OnInit {
  ...
  create_form(cliente:Cliente):void {
    ...
    this.form_cliente = new FormGroup({
      ...
      entRet: new FormControl(cliente.entRet, [Validators.required]),
      id_sabor: new FormControl(cliente.id_sabor, [Validators.required]),
  ...
  get_sabores():void {
    this.http
      .get('http://127.0.0.1:8000/api/sabores')
      .subscribe({
        next: (result) => {
          this.sabores = Object(result).data;
        },
        error: (e) => {
          console.log(e);
        }
